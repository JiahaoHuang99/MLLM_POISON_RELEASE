...
def evaluate_model(model, processor, dataset, args):
    ...
    for item in tqdm(dataset):
        # Note: image_path here is already resolved by the dataset's resolve_image_path method
...
def parse_args():
    ...
    # Wandb configuration
    parser.add_argument("--wandb_project", type=str, default="MIMIC-CXR-VQA-LoRA-QwenVL",
                       help="Wandb project name")
    parser.add_argument("--wandb_name", type=str, default="qwen25vl_lora_mimic_cxr_vqa_v1",
                       help="Wandb run name")
...
if __name__ == "__main__":
    ...
    train_dataset = MIMICCXRVQADataset(args.train_jsonl, processor, image_root=args.image_root, image_size=tuple(args.image_size))
    val_dataset = MIMICCXRVQADataset(args.val_jsonl, processor, image_root=args.image_root, image_size=tuple(args.image_size))
